...
# Uses the feedback mechanism and has the post-processing separated from the main compute
...
def make_sdfg(specialized):
    ...
    if specialized:
        sdfg = dace.SDFG("mm_systolic_feedback_output_{}_{}x{}x{}".format(
            P.get(), N.get(), K.get(), M.get()))
    else:
        sdfg = dace.SDFG("mm_systolic_feedback_{}_output_NxKx{}".format(P.get(), M.get()))
