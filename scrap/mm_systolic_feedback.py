...
# Just like the original but using the feedback mechanism needed for the split-up version
...
def make_sdfg(specialized):
    ...
    if specialized:
        sdfg = dace.SDFG("mm_systolic_feedback_{}_{}x{}x{}".format(
            P.get(), N.get(), K.get(), M.get()))
    else:
        sdfg = dace.SDFG("mm_systolic_feedback_{}_NxKx{}".format(P.get(), M.get()))
